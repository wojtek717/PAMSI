        ...
        for (int x = 0; x < 8; ++x) {
            if (boardArray[y][x].GetChequer().GetColor() == color) {
                if (this->boardArray[y][x].GetChequer().GetType() == man) {
                    ...
                    if ((y - 1 >= 0) && (x - 1 >= 0) &&
                        (this->IsMoveAvaliable(this->boardArray[y][x], this->boardArray[y - 1][x - 1]))) {
                    ...
                    for (int i = 1; i < 8; ++i) {
                        ...
                        if ((y - i >= 0) && (x - i >= 0) &&
                            (this->IsMoveAvaliable(this->boardArray[y][x], this->boardArray[y - i][x - i]))) {
                            this->boardArray[y][x].setAvaliable(true);
...
Capture BoardController::IsCaptureAvalible(Cell from, Cell dest) {
    ...
    if (from.GetChequer().GetType() == man) {
        if (dest.isFreeToMove() && (from.GetChequer().GetColor() != mid.GetChequer().GetColor()) && mid.isChequer()) {
            return Capture(from, dest, mid, true);
        } else {
            return Capture(from, dest, mid, false);
        }
    } else if (from.GetChequer().GetType() == king) {
        while (x != dest.GetX() && y != dest.GetY()) {
            ...
            if ((mid.GetChequer().GetColor() != from.GetChequer().GetColor()) &&
                (mid.GetChequer().GetColor() != noColor)) {
                enemy++;
            }
        }

        return Capture(from, dest, mid, enemy == 1 && dest.isFreeToMove());
    }
}
...
std::vector<Capture> BoardController::GetAvalibleCapture(Color color) {
    ...
    //Capture capture(this->boardArray[0][0], this->boardArray[0][0], this->boardArray[0][0], false);
        ...
        for (int x = 0; x < 8; ++x) {
            if (this->boardArray[y][x].GetChequer().GetColor() == color) {
                if (this->boardArray[y][x].GetChequer().GetType() == man) {
                    ...
                    if ((y - 2 >= 0) && (x - 2 >= 0) &&
                        (this->IsCaptureAvalible(this->boardArray[y][x], this->boardArray[y - 2][x - 2]).isCapture)) {
                        this->boardArray[y][x].setAvaliable(true);
                        isAnyCapture = true;
                        std::cout << "Avalible Capture: x=" << x << " y=" << y << std::endl;
                        Capture capture(this->boardArray[y][x], this->boardArray[y - 2][x - 2],
                                        this->boardArray[y - 1][x - 1], true);
                    ...
                    for (int i = 1; i < 8; ++i) {
                        ...
                        if ((y - i >= 0) && (x - i >= 0)) {
                            Capture capture = this->IsCaptureAvalible(this->boardArray[y][x],
                                                                      this->boardArray[y - i][x - i]);
...
void BoardController::MakeCapture(Cell from, Cell dest) {
    ...
    if (this->GetChosen().GetChequer().GetType() == man) {
        ...
        if ((dest.GetY() - 2 >= 0) && (dest.GetX() - 2 >= 0) &&
            (this->IsCaptureAvalible(this->boardArray[dest.GetY()][dest.GetX()],
                                     this->boardArray[dest.GetY() - 2][dest.GetX() - 2]).isCapture)) {
        } else if ((dest.GetY() - 2 >= 0) && (dest.GetX() + 2 <= 7) &&
                   (this->IsCaptureAvalible(this->boardArray[dest.GetY()][dest.GetX()],
                                            this->boardArray[dest.GetY() - 2][dest.GetX() + 2]).isCapture)) {
        } else if ((dest.GetY() + 2 <= 7) && (dest.GetX() - 2 >= 0) &&
                   (this->IsCaptureAvalible(this->boardArray[dest.GetY()][dest.GetX()],
                                            this->boardArray[dest.GetY() + 2][dest.GetX() - 2]).isCapture)) {
        } else if ((dest.GetY() + 2 <= 7) && (dest.GetX() + 2 <= 7) &&
                   (this->IsCaptureAvalible(this->boardArray[dest.GetY()][dest.GetX()],
                                            this->boardArray[dest.GetY() + 2][dest.GetX() + 2]).isCapture)) {
        } else {
            this->SwitchTurn();
        ...
        while (x != dest.GetX() && y != dest.GetY()) {
            ...
            if ((mid.GetChequer().GetColor() != this->GetChosen().GetChequer().GetColor()) &&
                (mid.GetChequer().GetColor() != noColor)) {
                this->Hide(mid.GetX(), mid.GetY());
                break;
            }
        ...
        for (int i = 1; i < 8; ++i) {
            ...
            if ((y - i >= 0) && (x - i >= 0) &&
                (this->IsCaptureAvalible(this->boardArray[y][x], this->boardArray[y - i][x - i]).isCapture)) {
                break;
            } else if ((y - i >= 0) && (x + i <= 7) &&
                       (this->IsCaptureAvalible(this->boardArray[y][x], this->boardArray[y - i][x + i]).isCapture)) {
                break;
